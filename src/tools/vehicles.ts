...
export async function resolveVehicleKey(
  ...
  const choices = vehicles
    .map((v) => `${v.vehicleKey} (${v.nickName ?? "unnamed"})`)
    .join(", ");
...
async function readVehicleInfo(
  client: KiaClient,
  vehicleKey: string,
): Promise<KiaVehicleInfo> {
  const info = await client.getVehicleStatus(vehicleKey, {
    includeClimate: true,
  });
  if (!info) {
    throw new McpToolError(
      `Kia returned no vehicle record for vehicle_key ${vehicleKey}.`,
      {
...
function describeSeats(
  seats: Record<string, KiaSeatHeatVent> | undefined,
...
export function registerVehiclesTools(
  server: McpServer,
  client: KiaClient,
): void {
  server.registerTool(
    ...
    async () => {
      ...
      return minifiedResult({
        ...
        vehicles: vehicles.map(summarizeVehicle),
      });
    },
  );
  ...
  server.registerTool(
    "kia_vehicle_status",
    {
      ...
      inputSchema: z.object({
        ...
        include_raw: z
          .boolean()
          .describe(
            "Include the full vehicleStatus block as `raw` (default false).",
          )
          .optional(),
      }),
    },
    async ({ vehicle_key, include_raw }) => {
      ...
      if (!status) {
        throw new McpToolError(
          `Kia returned no vehicleStatus for vehicle_key ${vehicleKey}.`,
          {
            hint:
      ...
      return minifiedResult({
        ...
        syncDateNote:
          "Advances on EVERY read whether or not anything changed — never treat it as proof of a change.",
  ...
  server.registerTool(
    ...
    async ({ vehicle_key }) => {
      ...
      if (!isRecord(location)) {
        throw new McpToolError(
          `Kia returned no location block for vehicle_key ${vehicleKey}.`,
          {
            hint:
